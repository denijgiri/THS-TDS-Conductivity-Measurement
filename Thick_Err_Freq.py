# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 10:00:46 2024
...
for i , (sub_ref, sub_sam, film_ref, film_sam) in enumerate(zip(*t)):
    ...
    a = (c * np.exp(1j*delta_sub) * delta_sub * R_sub_sam_bd *(-1)) / (np.exp(1j*delta_sam)*d_film*omega * R_film_sam_bd * d_sub **2)
    b = (c *delta_sub * (-1)) / (d_film*omega *d_sub **2)
    e =  (a - b )* (epsilon_0 * c)
    
   
    w = (2 *R_sub_sam_bd *  np.exp(1j*delta_sub)*(-1)) / (np.exp(1j *delta_sam)* R_film_sam_bd*(d_film)**2)
    x = (c * np.exp(1j *delta_sub) * delta_sub *R_sub_sam_bd* (-1)) / (np.exp(1j *delta_sam) * omega * d_sub * R_sub_sam_bd *d_film**2)
    v = 2 / (d_film)**2
    y = (c*delta_sub *(1)) / ((omega * d_sub * d_film**2))
    z = (w + x + v + y)* (epsilon_0 *c)

    
    thick_error = ((e*0.001)**2)+((z*0.001)**2)
    thick_error = np.sqrt(thick_error)
    thick_err.append(thick_error)
    #result.append(sigma)
...
print(avg_sigma)
thick_err = np.array(thick_err)
thick_err = np.mean(thick_err,axis = 0)
thick_err = thick_err[freqs_range]
print(thick_err)
